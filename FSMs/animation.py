...
class WalkingFSM(AnimateFSM):
    ...
    move = standing.to(moving) | jumping.to(moving)
    jump = standing.to(jumping) | moving.to(jumping)
    stop = moving.to(standing) | jumping.to(standing)
    # moveRight = standing.to(movingRight) | movingLeft.to(movingRight)
    # moveLeft = standing.to(movingLeft) | movingRight.to(movingLeft)
    # stop = movingRight.to(standing) | movingLeft.to(standing)
    ...
    def updateState(self):
        ...
        #     self.stop()
        if self.hasVelocity() and self.obj.velocity[0]!=0 and self.obj.velocity[1]==0 and self != "moving":
            self.move()
        elif self.hasVelocity() and self.obj.velocity[1]!=0 and self != "jumping":
            self.jump()
        elif not self.hasVelocity() and self != "standing":
            self.stop()
